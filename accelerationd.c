...
static int poll_sensor_data(struct sensors_poll_device_t *sensors_device)
{
	...
	struct dev_acceleration *passin = (struct dev_acceleration *)
		malloc(sizeof(struct dev_acceleration));
		passin->x = (int)(buffer[i].acceleration.x*100);
		passin->y = (int)(buffer[i].acceleration.y*100);
		passin->z = (int)(buffer[i].acceleration.z*100);
		syscall(378,passin);
	}
	return 0;
}
			...
			struct sensors_poll_device_t **mSensorDevice)
{
	...
	(*mSensorDevice)->setDelay(*mSensorDevice, list[i].handle, 10); 	
